...
public class MySlidingDrawer extends ViewGroup {
    ...
    private final boolean mRightTab;
    ...
    public MySlidingDrawer(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.MySlidingDrawer, defStyle, 0);

        mVertical = true;
        mBottomOffset = (int) a.getDimension(R.styleable.MySlidingDrawer_bottomOffset, 0.0f);
        mTopOffset = (int) a.getDimension(R.styleable.MySlidingDrawer_topOffset, 0.0f);
        mAllowSingleTap = true;
        mAnimateOnClick = a.getBoolean(R.styleable.MySlidingDrawer_animateOnClick, true);
        mRightTab = a.getBoolean(R.styleable.MySlidingDrawer_rightTab, true);
        mBackgroundColor = a.getColor(R.styleable.MySlidingDrawer_backgroundColor, 0);
        
        int handleId = a.getResourceId(R.styleable.MySlidingDrawer_handle, 0);
        if (handleId == 0) {
            throw new IllegalArgumentException("The handle attribute is required and must refer "
                    + "to a valid child.");
        }

        int contentId = a.getResourceId(R.styleable.MySlidingDrawer_content, 0);
        if (contentId == 0) {
    ...
    public void setmBottomOffset(int mBottomOffset) {
	...
	private static class SlidingHandler extends Handler {
		private final WeakReference<MySlidingDrawer> mService;

		SlidingHandler(MySlidingDrawer service) {
			mService = new WeakReference<MySlidingDrawer>(service);

		}

		public void handleMessage(Message m) {
			MySlidingDrawer service = mService.get();
			if (service != null) {
				switch (m.what) {
				case MSG_ANIMATE:
					service.doAnimation();
